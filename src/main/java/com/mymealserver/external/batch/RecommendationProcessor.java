...
import com.mymealserver.api.recommendation.service.dto.RecommendationResult.MealRecommendation;
...
public class RecommendationProcessor implements ItemProcessor<Member, List<Recommendation>> {
  ...
  public List<Recommendation> process(Member member) {
    ...
    // 5. 벡터 검색
    List<MealLog> goodMeals =
        mealLogReader.findSimilarMealsByVector(member.getId(), queryVector, 3);
    ...
    if (result.breakfast() != null && settings.getBreakfastTime() != null) {
      ...
      recommendations.add(
          createRecommendation(
              member.getId(), result.breakfast(), MealType.BREAKFAST, breakfastTime));
    }
    ...
    if (result.lunch() != null && settings.getLunchTime() != null) {
      ...
      recommendations.add(
          createRecommendation(member.getId(), result.lunch(), MealType.LUNCH, lunchTime));
    }
    ...
    if (result.dinner() != null && settings.getDinnerTime() != null) {
      ...
      recommendations.add(
          createRecommendation(member.getId(), result.dinner(), MealType.DINNER, dinnerTime));
    }
  ...
  private Recommendation createRecommendation(
      Long memberId, MealRecommendation mealRec, MealType mealType, LocalDateTime scheduledTime) {
    ...
    // 푸시 메시지 구성
    String pushMessage =
        String.format("🍽️ %s 추천: %s\n\n%s", mealType.getDescription(), mealName, reason);
  ...
  private boolean hasAnyTodayRecommendation(Long memberId, LocalDateTime start, LocalDateTime end) {
    return recommendationReader.existsRecommendationInRange(
            memberId, MealType.BREAKFAST, start, end)
  ...
  private String getQueryVector() {
    SearchPrompt searchPrompt =
        searchPromptReader
            .findActiveByType(PromptType.POSITIVE_SEARCH)
