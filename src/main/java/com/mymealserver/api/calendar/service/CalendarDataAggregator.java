...
public class CalendarDataAggregator {
  ...
  public Map<Long, Reaction> aggregateReactions(
  ...
  public List<Reaction> filterValidReactions(
      List<Meal> meals, Map<Long, Reaction> reactionsByMealId) {
    return meals.stream()
  ...
  public Double calculateAverageScore(List<Reaction> reactions) {
    ...
    return reactions.stream().mapToDouble(Reaction::getOverallScore).average().orElse(0.0);
  }
  ...
  public Set<MealType> extractMealTypes(List<Meal> meals) {
    return meals.stream().map(Meal::getMealType).collect(Collectors.toSet());
  }
}
