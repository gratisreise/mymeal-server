...
import com.mymealserver.api.auth.service.client.naver.NaverApiClient;
...
import com.mymealserver.api.auth.service.impl.NaverOAuthService;
...
@DisplayName("NaverOAuthService 단위 테스트")
class NaverOAuthServiceTest {
    ...
    class NewUserAuthenticationTests {
        ...
        private NaverUserInfoResponse userInfoResponse;
        ...
        void setUp() {
            ...
            tokenResponse = OAuthFixture.createNaverTokenResponse();
            userInfoResponse = OAuthFixture.createNaverUserInfo();
        ...
        void authenticate_WithNewUser_ShouldCreateMemberAndReturnTokens() {
            ...
            given(memberReader.findByProviderAndProviderId(eq(ProviderType.NAVER), eq(userInfoResponse.id()))).willReturn(null);
            ...
            // When
            naverOAuthService.authenticate(validRequest);

            // Then
            then(apiClient).should().exchangeCodeForToken(validRequest.code());
            then(apiClient).should().getUserInfo(tokenResponse.accessToken());
            then(memberReader).should().findByProviderAndProviderId(eq(ProviderType.NAVER), eq(userInfoResponse.id()));
        ...
        void authenticate_WithNullName_ShouldUseDefaultName() {
            // Given
            NaverUserInfoResponse userInfoWithNullName = OAuthFixture.createNaverUserInfoWithNullNickname();

            given(apiClient.exchangeCodeForToken(validRequest.code())).willReturn(tokenResponse);
            given(apiClient.getUserInfo(tokenResponse.accessToken())).willReturn(userInfoWithNullName);
            given(memberReader.findByProviderAndProviderId(eq(ProviderType.NAVER), eq(userInfoWithNullName.id()))).willReturn(null);
        ...
        @Test
        @DisplayName("신규 회원 생성 시 이메일 형식은 providerId@naver.com이다")
    ...
    class ExistingUserAuthenticationTests {
        ...
        void setUp() {
            ...
            testMember = Member.builder()
                    .id(1L)
                    .email(OAuthFixture.NAVER_USER_ID + "@naver.com")
                    .name(OAuthFixture.NAVER_USER_NAME)
                    .profileImage(OAuthFixture.NAVER_PROFILE_IMAGE)
                    .provider(ProviderType.NAVER)
                    .providerId(OAuthFixture.NAVER_USER_ID)
                    .isActive(true)
                    .build();
            mockAuthResponse = AuthResponse.builder()
                    .accessToken("test_access_token")
        ...
        void authenticate_WithExistingUser_ShouldUpdateLastLoginAndReturnTokens() {
            ...
            given(apiClient.exchangeCodeForToken(validRequest.code())).willReturn(tokenResponse);
            given(apiClient.getUserInfo(tokenResponse.accessToken())).willReturn(userInfoResponse);
            given(memberReader.findByProviderAndProviderId(eq(ProviderType.NAVER), eq(userInfoResponse.id()))).willReturn(testMember);
    ...
    class AuthenticationFailureTests {
        ...
        void authenticate_WithInvalidCode_ShouldThrowException() {
            ...
            // When & Then
            org.assertj.core.api.Assertions.assertThatThrownBy(() -> naverOAuthService.authenticate(invalidRequest))
        ...
        void authenticate_WhenUserInfoFails_ShouldThrowException() {
            ...
            // When & Then
            org.assertj.core.api.Assertions.assertThatThrownBy(() -> naverOAuthService.authenticate(validRequest))
                    .isInstanceOf(org.springframework.web.client.RestClientException.class);
    ...
    class FcmTokenTests {
        ...
        @Test
        @DisplayName("FCM 토큰이 없는 경우 FCM 토큰을 업데이트하지 않는는다")
        void authenticate_WithoutFcmToken_ShouldNotUpdateFcmToken() {
            ...
            // When
            naverOAuthService.authenticate(requestWithoutFcm);
    ...
    class ProviderTests {
        ...
        @Test
        @DisplayName("getProvider()는 NAVER를 반환한다")
        void getProvider_ShouldReturnNaver() {
            // When
            ProviderType provider = naverOAuthService.getProvider();

            // Then
            org.assertj.core.api.Assertions.assertThat(provider).isEqualTo(ProviderType.NAVER);
        }
    }
}
