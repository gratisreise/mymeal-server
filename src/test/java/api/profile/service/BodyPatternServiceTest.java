...
class BodyPatternServiceTest {
  ...
  void getBodyPatterns_success_withGoodAndBadTags() {
    // given
    List<FoodMemberStats> foodStats =
        List.of(
  ...
  void getBodyPatterns_success_nullOverallAverage() {
    // given
    List<FoodMemberStats> foodStats = List.of(createFoodMemberStats(1L, MEMBER_ID, 3.0, 3));
    given(foodMemberStatsRepository.findByMemberId(MEMBER_ID)).willReturn(foodStats);
  ...
  private FoodMemberStats createFoodMemberStats(
      Long id, Long memberId, double avgScore, int mealCount) {
