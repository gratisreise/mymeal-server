...
```bash
# 민감 정보 패턴 검색
grep -rn "password\s*=\s*\".*\"\|apiKey\s*=\s*\".*\"\|secret\s*=\s*\".*\"" --include="*.java" --include="*.properties" --include="*.yml"
grep -rn "-----BEGIN.*PRIVATE-----" .
...
**🚫 제외해야 할 파일:**
```
application-local.yml, application-secret.yml
*.pem, *.key, *.p12
...
- `application-local.yml`: password 포함
- `secrets.json`: API_KEY 포함
...
| `chore` | 빌드/설정 변경 | chore: Gradle 설정 업데이트 |
...
**변경된 파일:**
- src/main/java/.../service/AuthService.java (수정)
- src/test/java/.../AuthServiceTest.java (신규)
- docs/api.md (수정)

**제외할 파일:**
- application-local.yml (민감 정보)
...
```bash
# 개별 파일 스테이징 (권장)
git add src/main/java/.../service/AuthService.java src/test/java/.../AuthServiceTest.java docs/api.md
