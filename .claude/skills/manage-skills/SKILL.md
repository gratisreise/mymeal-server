...
- PR전에 verify 스킬이 변경된 영역을 커버하는지 확인할 때
...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-api-design` | RESTful API 설계 원칙과 엔드포인트 명명 규칙 검증 | `src/main/java/**/controller/**/*`, `src/main/java/**/api/**/*` |
| `verify-database` | DB 쿼리 성능, SQL 인젝션, N+1 문제 검증 | `src/main/java/**/entity/**/*`, `src/main/java/**/repository/**/*`, `src/main/resources/db/migration/**` |
| `verify-security` | 인증/인가, 민감 정보, 입력 검증 보안 검증 | `src/main/java/**/config/SecurityConfig*`, `src/main/java/**/dto/request/**/*` |
| `verify-error-handling` | 예외 처리, 에러 응답, 로깅 일관성 검증 | `src/main/java/**/exception/**/*`, `src/main/java/**/handler/**/*ExceptionHandler*` |
| `verify-performance` | 캐싱, 커넥션 풀, 비동기 처리 최적화 검증 | `src/main/java/**/config/*Config*`, `src/main/resources/application*.yml` |
| `verify-testing` | 단위/통합 테스트, 커버리지 검증 | `src/test/java/**/*Test*`, `src/test/java/**/*Tests*` |
...
| 디렉토리 | 파일 |
|----------|------|
| src/main/java/.../controller | `UserController.java`, `OrderController.java` |
| src/main/java/.../service | `UserService.java` |
| src/test/java | `UserServiceTest.java` |
| (루트) | `pom.xml`, `application.yml` |
```
...
| 스킬 | 트리거 파일 (변경된 파일) | 액션 |
|------|--------------------------|------|
| verify-api-design | `UserController.java`, `OrderController.java` | CHECK |
| verify-security | `SecurityConfig.java` | CHECK |
| (스킬 없음) | `pom.xml`, `application.yml` | UNCOVERED |
```
...
3. **커버되지 않은 새 패턴** — 변경된 파일을 읽고 스킬이 검사하지 않는 새로운 규칙, 설정, 패턴을 식별합니다. 확인 사항:
   - 새로운 어노테이션, enum 변형, 또는 exported 심볼
...
| 스킬 | 갭 유형 | 상세 |
|------|---------|------|
| verify-api-design | 파일 누락 | `src/main/java/.../handler/NewHandler.java`가 Related Files에 없음 |
| verify-security | 새 패턴 | 새 컨트롤러가 검사되지 않는 @PreAuthorize 규칙을 사용 |
| verify-testing | 오래된 값 | 테스트 설정 파일의 패턴이 변경됨 |
```
...
**결정: 기존 스킬 UPDATE** (N개)
- `verify-api-design` — 누락된 파일 참조 2개 추가, 탐지 패턴 업데이트
- `verify-testing` — 새 설정 패턴에 대한 탐지 명령어 업데이트
...
|------|---------|
| ... 기존 항목 ... |
| `src/main/java/.../handler/NewHandler.java` | 유효성 검사가 포함된 새 요청 핸들러 |
```
...
**파일:** `src/main/java/.../handler/NewHandler.java`

**검사:** 검증할 내용에 대한 설명.

```bash
grep -n "@NewAnnotation" src/main/java/.../handler/NewHandler.java
```
...
1. **Lock 파일 및 생성된 파일** — `pom.xml`의 dependency 섹션, `gradle.lockfile`, 자동 생성된 마이그레이션 파일, 빌드 출력물은 스킬 커버리지가 불필요
2. **일회성 설정 변경** — `pom.xml`/`build.gradle`의 버전 범프, 린터/포매터 설정의 사소한 변경은 새 스킬이 불필요
3. **문서 파일** — `README.md`, `CHANGELOG.md`, `LICENSE` 등은 검증이 필요한 코드 패턴이 아님
4. **테스트 픽스처 파일** — 테스트 픽스처로 사용되는 디렉토리의 파일(예: `fixtures/`, `test-resources/`)은 프로덕션 코드가 아님
...
8. **CI/CD 설정** — `.github/`, `.gitlab-ci.yml`, `Dockerfile`, `Jenkinsfile` 등은 인프라이며, 검증 스킬이 필요한 애플리케이션 패턴이 아님
